...
override 'do_check' => sub {
    ...
    my $res = $self->call_ma($params->ma_url, $params->source, $params->destination, $params->time_range, $params->metric, $params->timeout, $stats);
    return ($res, $stats) if($res);
    if($params->bidirectional){
        $res = $self->call_ma($params->ma_url, $params->destination, $params->source, $params->time_range, $params->metric, $params->timeout, $stats);
        return ($res, $stats) if($res);
    }
    return ('', $stats);
};
...
sub call_ma {
    #send request
    my ($self, $ma_url, $src, $dst, $time_int, $metric, $timeout, $stats) = @_;
